...
const validateNumber = z
  ...
  .or(
    z
      .string()
      .regex(/^$|^[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)$/, {
        message: "Expected number, recieved string",
      })
...
const schema = z.object({
  ...
  roastProfile: z.object({
    ...
    firstCrack: z
      .string()
      .min(1, { message: "Required" })
      .regex(/^(([0]?[0-5][0-9]|[0-9]):([0-5][0-9]))$/, {
        message: "mm:ss format required",
      }),
    rollingFirstCrack: z
      .string()
      .min(1, { message: "Required" })
...
function RoastLog(props) {
  ...
  const getRoastLog = async (id) => {
    try {
      ...
      console.error("Error while trying to get roast log: ", err);
      setError(err);
    }
  };
  ...
  const deleteRoastLog = async (roastId) => {
    try {
      ...
      console.error("Error while trying to delete roast log: ", err);
    }
  };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      console.error("Error occurred on submit: ", err);
    }
  };
  ...
  const updateRoastLog = async (roastId, data) => {
    try {
      ...
      console.error("Error updating roast log: ", err);
    }
  };

  const handleDelete = async () => {
    try {
      await deleteRoastLog(roastLog._id);
      togglePopup();
      setIsDeleted(!isDeleted);
    } catch (err) {
      console.log(`Error while trying to delete roast log: ${err}`);
    }
  };
    ...
    <>
      {roastLog === null && error === null ? (
        ...
        <div className="alertDeleted center">
          ...
          <Link className="primaryBtn linkBtn" to="/">
            Home
          </Link>
          <Link className="primaryBtn linkBtn" to="/mantraCoffee/add-roast-log">
            Log New Roast!
          </Link>
        </div>
      ) : (
          ...
          >
            ...
            <div className="fg-item fg-left">
              ...
              >
                ...
                <option value="Light - Medium Brown">
                  Light - Medium Brown
                </option>
            ...
            {isDisabled ? (
              <div className="btn-container">
                <button
                  className="primaryBtn space-between"
                  type="button"
                  onClick={handleEdit}
                >
                  Edit
                  ...
                  <>
                    {isDeletePopup ? (
                      <>
                        ...
                        <p>
                          If you do not wish to DELETE this roast log click
                          CANCEL. To confirm DELETION click delete.
                        </p>
                        <button
                      ...
                      <>
                        ...
                        <p>
                          Once UPDATED all previous data will be replaced with
                          the current form values. If you do not wish to UPDATE
                          click CANCEL. To UPDATE click UPDATE.
                        </p>
                        <button
                          className="primaryBtn"
                          type="button"
