...
function RoastDataTable(props) {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: (props) => {
          return (
            <Link to={"/mantraCoffee/" + props.value}>
              <button className="primaryBtn">View Log!</button>
            </Link>
          );
        },
      },
  ...
  if (roastLogs.length < 1 && isDataLoading === false) {
    return (
      <h2 className="noRoasts">
        Woah! Looks Like you have no Roasts Logged! Try Logging your first roast
        by clicking the Start New Roast Button.
      </h2>
    );
  }
  return (
